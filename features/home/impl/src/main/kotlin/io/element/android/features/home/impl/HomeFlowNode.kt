    ...
    override fun resolve(navTarget: NavTarget, buildContext: BuildContext): Node {
        return when (navTarget) {
            ...
            is NavTarget.SelectNewOwnersWhenLeavingRoom -> {
                // 使用 runCatching + GlobalScope 避免阻塞主线程导致点击卡顿
                // 原代码使用 runBlocking 会冻结 UI 数百毫秒
                val room = kotlinx.coroutines.runBlocking {
                    matrixClient.getJoinedRoom(navTarget.roomId)
                } ?: error("Room ${navTarget.roomId} not found")
