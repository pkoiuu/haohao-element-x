...
class VectorUnifiedPushMessagingReceiver : MessagingReceiver() {
    ...
    override fun onMessage(context: Context, message: PushMessage, instance: String) {
        ...
        Timber.tag(loggerTag.value).d("New message, decrypted: ${message.decrypted}, size: ${message.content.size}")

        // HaohaoChat: 使用 goAsync() + runBlocking 确保 BroadcastReceiver 有足够时间处理消息
        // 原代码使用 coroutineScope.launch，但当进程被系统杀死时协程会被取消，
        // 导致 pushHandler.handle 未执行，WorkManager 任务未提交
            ...
            try {
                runBlocking {
                    ...
                    if (pushData == null) {
                        ...
                        Timber.tag(loggerTag.value).d("Push data parsed, handling...")
                        val handled = pushHandler.handle(
                            pushData = pushData,
                            providerInfo = "${UnifiedPushConfig.NAME} - $instance",
                        )
                        Timber.tag(loggerTag.value).d("Push handled: $handled")
                ...
                Timber.tag(loggerTag.value).e(e, "Error handling push message")
            } finally {
