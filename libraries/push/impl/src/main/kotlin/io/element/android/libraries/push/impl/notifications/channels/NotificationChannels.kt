...
// HaohaoChat: V3 — 渠道 importance 从 DEFAULT 提升为 HIGH，支持 heads-up 弹出通知
...
class DefaultNotificationChannels(
    ...
    private fun createNotificationChannels() {
        ...
        // HaohaoChat: 迁移 V2 noisy 渠道到 V3（importance 从 DEFAULT 提升为 HIGH）
        // Android 不允许提升已创建渠道的 importance，必须创建新渠道
        for (channel in notificationManager.notificationChannels) {
            if (channel.id.startsWith("DEFAULT_NOISY_NOTIFICATION_CHANNEL_ID_V2")) {
    ...
    private fun buildNoisyChannel(channelId: String, soundUri: Uri?, accentColor: Int): NotificationChannelCompat {
        // HaohaoChat: IMPORTANCE_HIGH 支持 heads-up 弹出通知（横幅通知）
        // 原 IMPORTANCE_DEFAULT 只在通知栏显示，不弹出
