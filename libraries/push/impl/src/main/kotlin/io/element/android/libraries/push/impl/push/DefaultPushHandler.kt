...
class DefaultPushHandler(
    ...
    private suspend fun handleInternal(pushData: PushData, providerInfo: String): Boolean {
        try {
            ...
            // 始终提交 Work，依赖 ExistingWorkPolicy.APPEND_OR_REPLACE 进行去重和链式执行
            // 修复：hasPendingWork 检查会在 Worker 运行中阻止新 push 触发处理，
            // 导致后续消息只存入数据库但不被处理
            Timber.d("Submitting notification sync work for session $userId")
