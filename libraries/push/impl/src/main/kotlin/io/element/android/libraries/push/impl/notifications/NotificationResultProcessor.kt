...
class DefaultNotificationResultProcessor(
    ...
    override fun start() {
        ...
        // HaohaoChat: 添加 retry 机制，确保 processResults 抛出异常后 Flow collector 能自动重启
        // 否则一旦 processJob 死亡，后续所有 emit() 的数据都会被缓冲但无人消费
        processJob = resultFlow
            .onEach(::processResults)
            .retry { cause ->
                Timber.tag(TAG).e(cause, "processResults threw exception, restarting Flow collector")
                true
            }
    ...
    private suspend fun processResults(results: Map<PushRequest, Result<ResolvedPushEvent>>) {
        // HaohaoChat: 包裹 try-catch 防止异常杀死 Flow collector
        // processJob 一旦死亡，后续所有 emit() 的数据会被缓冲但无人消费
        try {
            ...
            val event = result.recover { exception ->
                ...
                when (exception) {
                    is NotificationResolverException.EventFilteredOut -> {
                        // HaohaoChat: 事件被过滤（可能是 sync 缓存导致），改为显示 fallback 通知
                        // 而非完全跳过，确保用户始终收到推送提醒
                        Timber.tag(TAG).w("Event filtered out, showing fallback notification for event ${request.eventId}")
                        ResolvedPushEvent.Event(
                            fallbackNotificationFactory.create(
                                sessionId = SessionId(request.sessionId),
                                roomId = RoomId(request.roomId),
                                eventId = EventId(request.eventId),
                                cause = "Event filtered out",
                            )
                        )
                    }
        ...
        if (nonRingingCallEvents.isNotEmpty()) {
            ...
            Timber.tag(TAG).e(e, "Error processing notification results, will retry on next emit")
        }
    } // end processResults
