...
class DefaultNotificationCreator(
        ...
        return builder
            ...
            .apply {
                ...
                if (roomInfo.shouldBing) {
                    // HaohaoChat: PRIORITY_HIGH 触发 heads-up 弹出通知
    ...
    override fun createFallbackNotification(
        ...
        // HaohaoChat: 使用 noisy 渠道而非 silent 渠道，确保 fallback 通知有声响和弹出
        // 原代码使用 noisy=false (SILENT channel, IMPORTANCE_LOW)，导致 fallback 通知完全静默
        val channelId = notificationChannels.getChannelIdForMessage(
        ...
        return NotificationCompat.Builder(context, channelId)
            // HaohaoChat: 每次都提醒（声音+振动），原 setOnlyAlertOnce(true) 导致只提醒一次
